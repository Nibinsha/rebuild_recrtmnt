...
class CandidateList(LoginRequiredMixin,ListView):
    login_url='/login/'
...
class CandidateEditView(UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        can.valid_up_to = cd.get('valid_up_to') if cd.get('valid_up_to') else None
        ...
        # can.interview = inter
        try:
            clientData = ClientMaster.objects.get(id=cd.get('intervew_company'))
            can.intervew_company = clientData
        except:
            pass
        if cd.get('scheduled_on') == "":

            date = None
        else:
            date = cd.get('scheduled_on')

       # can.intervew_company = clientData
        can.scheduled_on = date
